...
extension NetworkManager {
    /// Function to set up a URL with query parameters including time and date.
    ///
    /// - Parameters:
    ///   - from: The starting location (as a string).
    ///   - to: The destination location (as a string).
    ///   - date: The selected date for the trip in "YYYY-MM-DD" format.
    ///   - time: The selected time for the trip in "HH:MM" format.
    /// - Returns: A URL constructed with the provided parameters.
    func setupURLForSearchPoints(from: String, to: String, formattedDate: String, time: String? = nil) -> URL? {
        // Encoding parameters to be URL-safe.
        let characterSet = CharacterSet.urlQueryAllowed
        let fromText = from.addingPercentEncoding(withAllowedCharacters: characterSet) ?? ""
        let toText = to.addingPercentEncoding(withAllowedCharacters: characterSet) ?? ""
        
        // Constructing the base URL string
        var urlString = "https://timetable.search.ch/api/route.json?from=\(fromText)&to=\(toText)&date=\(formattedDate)"
        
        // Adding optional parameters if provided
        if let time = time?.addingPercentEncoding(withAllowedCharacters: characterSet) {
            urlString += "&time=\(time)"
        }
    ...
    /// Generic function to perform a network request with date and time, decoding the response into a data model.
    ///
    /// - Parameters:
    ///   - url: The URL for the network request.
    ///   - date: The selected date in "YYYY-MM-DD" format.
    ///   - time: The selected time in "HH:MM" format.
    ///   - isFetching: A closure to indicate whether fetching has started or stopped.
    ///   - completion: A closure with a `Result` type containing either a success with the decoded data or a failure with an error.
    ///   - T: The expected model type that conforms to `Decodable`.
    func performRequestForSearchPoints<T: Decodable>(with url: URL, isFetching: @escaping (Bool) -> Void, completion: @escaping (Result<T, Error>) -> Void) {
        // Indicate that fetching has started.
        isFetching(true)
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                // Print the raw JSON response for debugging purposes.
                if let jsonString = String(data: data, encoding: .utf8) {
                    print("Selected MAIN JSON Response: \(jsonString)")
                }
